...
{ 
   ...
   timer_start(0);
   sdc_init();   
   init(); // produce P0 , function init() is in kernel.c !!!

   kfork("/bin/init"); // produce P1 , function kfork() is in fork.c !!!
   
   kprintf("P0 switch to P1 : enter a line : ");
   kgetline(line);
 
   /* switch to run P1 ==> never return again  !!! */
   tswitch();  // function tswitch() is in ts.s !!!
}
