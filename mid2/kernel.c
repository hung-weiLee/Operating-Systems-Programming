...
int ktime(int time)
{
  ...
  if(head->next == 0)
  {
    head->next = tqe; // add first(head->next)
  }
  else
  {
    TQE *k = head;
    ...
    while(k->next != 0)
    {
      if(tqe->time > k->next->time)
      {
        tqe->time -= k->next->time;
        k = k->next;
      }
      else // add middle
      {
        k->next->time -= tqe->time;

        tqe->next = k->next;
        k->next = tqe;
        ksleep(tqe->pid); return;
      }
    }
    k->next = tqe; // add last
  }

  ksleep(tqe->pid); // sleep process
}

int time_update() /* (will excute every second) !!! */
{
  printTime_list(); // print timer_list 

  head->next->time--;
  if(head->next->time <= 0)
  {
    kwakeup(head->next->pid); // when the time finish , wakeup process
...
int body(int pid1, int ppid, int func, int priority)
{
  ...
  while(1)
  {
    ...
    switch(c)
    {
      ...
      case 't': kprintf("Enter time: "); c = kgetc();
                while(c != '\r')
                {
                  kprintf("%c", c);
                  arr[i] = c;
