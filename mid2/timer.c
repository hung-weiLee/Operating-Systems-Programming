...
{
    u32 ris, mis, value, load, bload, i,j;
    TIMER *t = &timer[n];

    t->tick++;

  /* to turn off/on the cursor every 0.5 seconds to make it blinking  !!! */

    if(t->tick % 64 == 0) // 0.5 second = 64 microsecond
    {
      if (flag1)
      {
        putcursor(cursor); /* open cursor */
        flag1 = 0;
      } 
      else 
      {
        clrcursor(); /* close cursor */
        flag1 = 1;
      }
    }

    if (t->tick >= 256) // 128 microsecond = 1 second 
    {
      t->tick = 0; t->ss++;

      /* call time_update function  ==>  (will excute every second)  !!! */
      time_update(); 

      if (t->ss >= 60) // 60 second = 1 minute
      {
	      t->ss = 0; t->mm++;
	      if (t->mm >= 60) // 60 minute = 1 hour
        {
	        t->mm = 0; t->hh++;
	      }
      }
