...
{
  int SR = int_off();
  s->value--; // empty's value decrease
  if (s->value < 0)
      block(s); // buffer is full => call block function
  int_on(SR);
}
...
int block(struct semaphore *s)
{
  ...
  tswitch(); /* tswitch will call (scheduler function) and (p2 function) !!! */
}
